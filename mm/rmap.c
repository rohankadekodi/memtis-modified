	...
	unsigned long address, void *arg)
{
	...
	if (pvmw.pte) {
	    ...
	    unsigned long prev_accessed, prev_idx, cur_idx, accesses;
		...
		prev_idx = compute_idx(hca->memcg->nr_sampled, hca->memcg->last_cooling_sample, pginfo->recent_accesses, pginfo->bottom_accesses, pginfo->bottom_accesses, htmm_cooling_period, htmm_mode, htmm_bp_cooling_factor, 0);
		...
		cur_idx = compute_idx(hca->memcg->nr_sampled, hca->memcg->last_cooling_sample, pginfo->recent_accesses, pginfo->bottom_accesses, pginfo->bottom_accesses, htmm_cooling_period, htmm_mode, htmm_bp_cooling_factor, 0);
		...
		if (cur_idx >= (hca->memcg->active_threshold - 1)) {
		    hca->page_is_hot = 2; 
		    if (!PageActive(page)) {
			    bpf_register_page_add_to_mig_queue_promotion(address, phase_num, pginfo->recent_accesses, pginfo->bottom_accesses);
		    }
		} else {
		    hca->page_is_hot = 1;
		    if (PageActive(page)) {
			    bpf_register_page_add_to_mig_queue_demotion(address, phase_num, pginfo->recent_accesses, pginfo->bottom_accesses);
		    }
		}
		if (prev_idx >= (hca->memcg->bp_active_threshold))
		    pginfo->may_hot = true;
		else
	...
	unsigned long address, void *arg)
{
	...
	if (pvmw.pte) {
	    ...
	    hca->lifetime_accesses = pginfo->bottom_accesses;
	} else if (pvmw.pmd) {
...
static bool get_pginfo_stm_accesses_one(struct page *page, struct vm_area_struct *vma,
	unsigned long address, void *arg)
{
    struct htmm_accesses_arg *hca = arg;
    struct page_vma_mapped_walk pvmw = {
	...
	if (pvmw.pte) {
	    ...
	    hca->lifetime_accesses = pginfo->recent_accesses;
	} else if (pvmw.pmd) {
	    hca->lifetime_accesses = 0;
	}
    }
...
unsigned long get_pginfo_stm_accesses(struct page *page)
{
    ...
    struct rmap_walk_control rwc = {
	.rmap_one = get_pginfo_stm_accesses_one,
	.arg = (void *)&hca,
    };
...
unsigned long get_pginfo_accesses_per_mig(struct page *page)
{
    ...
    struct rmap_walk_control rwc = {
	.rmap_one = get_pginfo_accesses_per_mig_one,
	.arg = (void *)&hca,
    };
...
unsigned long get_pginfo_lifetime_accesses(struct page *page)
{
    ...
    struct rmap_walk_control rwc = {
	.rmap_one = get_pginfo_lifetime_accesses_one,
	.arg = (void *)&hca,
    };
...
static bool get_page_virtual_address_one(struct page *page, struct vm_area_struct *vma,
	unsigned long address, void *arg)
{
    struct htmm_virtual_address_arg *hca = arg;
    hca->virtual_address = address;
    return true;
}

unsigned long get_page_virtual_address(struct page *page)
{
    struct htmm_virtual_address_arg hca = {
	.virtual_address = 1,
    };
    struct rmap_walk_control rwc = {
	.rmap_one = get_page_virtual_address_one,
	.arg = (void *)&hca,
    };
